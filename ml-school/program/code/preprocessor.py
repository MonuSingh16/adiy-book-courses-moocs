...
def preprocess(base_directory):
    ...
    y_train = target_transfomer.fit_transform(np.array(df_train.species.values).reshape(-1, 1))
    y_validation = target_transfomer.transform(np.array(df_validation.species.values).reshape(-1, 1))
    y_test = target_transfomer.transform(np.array(df_test.species.values).reshape(-1, 1))
    ...
    def _save_model(base_directory, features_transformer, target_transfomer):
        """ 
        ...
        with tempfile.TemporaryDirectory() as directory:
            joblib.dump(target_transfomer, os.path.join(directory, "target.joblib"))
